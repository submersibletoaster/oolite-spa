...
has_mixed_economy(X):- planet_economy(X,E),lists:memberchk('Mainly',E).

earthlike_set(X) :- list_to_ord_set(['plantations','oceans','forests','tree'],X).
scorched_set(X) :- olist_to_ord_set(['solar','war'],X).

is_gasgiant(X):-
		planet_economy(X,E),
                lists:memberchk('Industrial',E),
		planet_radius(X,R),
		R > 6870.  % thankyou cim
...
is_earthlike(X):-
		...
		planet_economy(X,E),
		lists:memberchk('Agricultural',E).
